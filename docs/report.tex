...
All scenarios were tested on a mesh, where the handle region contains 496 and the support region 1569 vertices. The measured times are the average of 500 measured values. \\
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|}
		\hline
		without details & 0.063 ms \\
		with details & 1.153 ms \\
		\hline
	\end{tabular}
\label{tab:handle}
\end{table}

Apart from the position changes, you can also make changes that change the deformation operator. These changes can only be used with a slight delay.

\begin{table}[h]
	\centering
	\caption{As we see, the higher the order of the operator, the longer the algorithm takes to adapt the operator}
	\begin{tabular}{|l|l|}
		\hline
		order 1 & 11.989 ms \\
		order 2 & 87.990 ms \\
		order 3 & 212.766 ms \\
		\hline
		details order 1 & 15.349 ms \\
		details order 2 & 73.921 ms \\
		details order 3 & 166.362 ms \\
		\hline
	\end{tabular}
\label{tab:order}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{|l|l|}
		...
		\hline
		smoothness handle & 240.960 ms \\
		smoothness boundary & 246.254 ms \\		
		\hline
	\end{tabular}
\label{tab:order}
\end{table}
